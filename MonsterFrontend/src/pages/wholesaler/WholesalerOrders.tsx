  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">Bulk Order History</h1>
        <span className="text-gray-500">{orders.length} bulk orders found</span>
      </div>

      {orders.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed">
          <p className="text-gray-500 text-lg">Aapne abhi tak koi bulk order nahi kiya hai.</p>
        </div>
      ) : (
            ...
            <div key={index} className="bg-white border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="bg-gray-50 px-6 py-4 border-b flex flex-wrap justify-between gap-4">
                <div>
                  ...
                  <p className="font-medium text-sm">#BULK-{Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
                </div>
                <div>
                    ...
                    <li key={idx} className="py-3 flex justify-between items-center">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">Qty: {item.quantity} units (Bulk Order)</p>
                        </div>
                      </div>
